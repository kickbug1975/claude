    ...
    private async executeItem(item: SyncItem) {
        switch (item.action) {
            case 'CREATE_FEUILLE':
                ...
                await api.post('/api/feuilles', payloadWithoutId);
                break;
            case 'UPDATE_FEUILLE':
                await api.put(`/api/feuilles/${item.payload.id}`, item.payload.data);
                break;
            case 'DELETE_FEUILLE':
                await api.delete(`/api/feuilles/${item.payload.id}`);
                break;
        }
    }

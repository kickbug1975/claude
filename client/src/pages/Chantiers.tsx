          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredChantiers.map((chantier) => (
                <div key={chantier.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow bg-white">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className={`px-2 py-0.5 text-xs rounded-full whitespace-nowrap ${chantier.actif ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                          }`}>
                          {chantier.actif ? 'En cours' : 'Terminé'}
                        </span>
                      </div>
                      <p className="text-sm text-gray-500 truncate">{chantier.reference}</p>
                    </div>
                  </div>
